...
| [0032](0032-free-form-training-data-store.md) | Free-form training-data store | Relocate free-text (PM goal/reason, model verifier prose) out of `events.payload` into a local `event_free_form` store, retained as training data; log stays body-free (approvals.reason bounded exception kept). |
| [0033](0033-human-remote-ops-controlplane.md) | Human remote ops control-plane (temporary) | Runtime services on a compose `runtime` profile; Spokesman gets the docker socket + a token-gated, human-fast-path, audited `ops` control-plane (named allowlist + gated `docker` escape hatch, destructive-confirm). Explicitly temporary scaffolding; LLM can never invoke it. |
