...
(M3b), the PM → Executor → Verifier → **Reviewer/Whistle-blower** → Retro roles +
worker (M3c/M7), four-layer memory (M4), search gateway (M5), and the skills layer.
...
Evidence: `DATABASE_URL=… pytest runtime/tests/` = **276 passed, 0 skips**;
`python -m runtime.demo` exits 0 (PM decomposes into >1 work items, all verified
done; the learning loop distills + injects a lesson; the Reviewer passes a clean
episode and flags + escalates a hallucinated-success one).
